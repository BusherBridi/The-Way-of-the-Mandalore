...
void astroidTimer(int id)
{
    ...
    glutTimerFunc(1000, astroidTimer, id);
}
void astroidSpawner(int id)
{
    if (singleton->astroids.size() < 4)
    {
        ...
        if (rand() % 3 == 0)
        {
            float yCoord = -1 + static_cast <float> (rand()) /( static_cast <float> (RAND_MAX/(1-(-1))));
            
            singleton->astroids.push_back(new Astroid(1, yCoord, 0.2, 0.2, true, 3, 0.005, 2));
        }
    }
...
{
    razorcrest = new SpaceShip("images/razocrestFlight.png", 5, 1, 0, 0, 0.6, 0.3, true, 10, 0.01, 3, 0.6, -0.05);
    xwings.push_back(new SpaceShip("images/xwingFlight.png", 1, 6, -1.7, 0.2, 0.6, 0.3, true, 10, 0.005, 3, 0.4, 0));
    xwings.push_back(new SpaceShip("images/xwingFlight.png", 1, 6, -1.7, -0.12, 0.6, 0.3, true, 10, 0.005, 3, 0.4, 0));
